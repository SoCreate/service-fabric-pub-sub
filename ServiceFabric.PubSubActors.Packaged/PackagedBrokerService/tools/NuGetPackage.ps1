...
function HandlePublishError {
	...
	$setupTask = Start-Process PowerShell.exe "-ExecutionPolicy Unrestricted -File .\tools\NuGetSetup.ps1 -Url $url -Base64EncodedMessage $encodedMessage" -Wait -PassThru

	#Write-Log ("NuGet Setup Task Exit Code: " + $setupTask.ExitCode)

	if ($setupTask.ExitCode -eq 0) {
		# Try to push package again
		$publishTask = Create-Process .\tools\NuGet.exe ("push " + $_.Name + " -Source " + $url)
...
function Run-Process-Wait-For-Message{
...
function Publish {
	...
	# Get nuget config
	[xml]$nugetConfig = Get-Content .\tools\NuGet.Config
	...
	$nugetConfig.configuration.packageSources.add | ForEach-Object {
		...
		Get-ChildItem *.nupkg | Where-Object { $_.Name.EndsWith(".symbols.nupkg") -eq $false } | ForEach-Object { 
			...
			# Try to push package
			$task = Create-Process .\tools\NuGet.exe ("push " + $_.Name + " -Source " + $url)
			$task.Start() | Out-Null
			$task.WaitForExit()
